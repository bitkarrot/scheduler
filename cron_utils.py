...
async def save_job_execution(response: str, jobID: str, adminkey: str) -> None:
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info("jobID: %s, status_code: %s", jobID, response.status_code)
            ...
            logger.info('pushdb: now pushing execution data to the database for jobID: %s', jobID)
            ...
            logger.info('SaveJobExecution: pushdb status: %s', pushdb_response.status_code)
            # logger.info(f'SaveJobExecution: pushdb text: {pushdb_response.text}')

            if pushdb_response.status_code == 200:
                logger.info('success: saved results to db for jobID: %s', jobID)
                return True
    except Exception as e:
        logger.error('error, saving to database for jobID: %s', jobID)
        logger.error(e)
        return False
...
async def get_job_by_id(jobID: str, adminkey: str):
    ...
    try:
        ...
        logger.info("[get_job_by_id]: response items in get_job_by_id: %s\n", response.text)
